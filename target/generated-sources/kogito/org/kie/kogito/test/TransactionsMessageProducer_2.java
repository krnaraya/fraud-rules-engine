...
public class TransactionsMessageProducer_2 {

    @javax.inject.Inject()
    @io.smallrye.reactive.messaging.annotations.Channel("automodedecisions")
    ...
    private String marshall(ProcessInstance pi, org.redhat.appdev.Transaction eventData) {
        try {
            if (useCloudEvents.orElse(true)) {
                TransactionsMessageDataEvent_2 event = new TransactionsMessageDataEvent_2("", eventData, pi.getId(), pi.getParentProcessInstanceId(), pi.getRootProcessInstanceId(), pi.getProcessId(), pi.getRootProcessId(), String.valueOf(pi.getState()));
