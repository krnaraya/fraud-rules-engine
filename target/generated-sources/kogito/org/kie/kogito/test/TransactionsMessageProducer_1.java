...
public class TransactionsMessageProducer_1 {

    @javax.inject.Inject()
    @io.smallrye.reactive.messaging.annotations.Channel("displaytransactions")
    ...
    private String marshall(ProcessInstance pi, org.redhat.appdev.Transaction eventData) {
        try {
            if (useCloudEvents.orElse(true)) {
                TransactionsMessageDataEvent_1 event = new TransactionsMessageDataEvent_1("", eventData, pi.getId(), pi.getParentProcessInstanceId(), pi.getRootProcessInstanceId(), pi.getProcessId(), pi.getRootProcessId(), String.valueOf(pi.getState()));
